...
from plotly_integration.models import MassCheckResult,MassCheckReport
from datetime import datetime
from django.utils.timezone import is_aware


app = DjangoDash("CreateMassCheckReportApp")

# Fetch available results
results = MassCheckResult.objects.all().order_by("-uploaded_at")
columns = [
...
app.layout = html.Div(
    ...
    children=[
        html.H1("Mass Check Report Creator", style={"textAlign": "center", "color": "#0047b3", "marginBottom": "20px"}),
...
def update_experiment_options(selected_sample_type):
    ...
    experiment_numbers = (
        MassCheckResult.objects.filter(sample_type=selected_sample_type)
...
def update_table(selected_sample_type, selected_experiment_numbers):
    query = MassCheckResult.objects.all().order_by("-uploaded_at")  # ✅ Sort by most recent first
...
# ✅ Create MassCheckReport Report Callback
@app.callback(
...
def create_report(n_clicks, report_name, project_id, user_id, comments, table_data, selected_rows):
    ...
    MassCheckReport.objects.create(
        report_name=report_name,
        project_id=project_id,
        user_id=user_id,
        comments=comments,
        selected_result_ids=",".join(map(str, selected_result_ids)),
        selected_result_names=",".join(map(str, selected_result_names))
    )

    return "✅ MassCheckReport Report created successfully!"

...
class DatabaseManagerWithTunnel {
    ...
    async insertAktaNodeIds(data) {
        ...
        const query = `
            INSERT IGNORE INTO akta_node_ids (result_id, run_log, fraction, uv_1, uv_2, uv_3, cond,
    ...
    async batchInsertAktaNodeIds(dataArray) {
        ...
        // Fixed query - VALUES needs parentheses for batch insert
        const query = `
            INSERT IGNORE INTO akta_node_ids (result_id, run_log, fraction, uv_1, uv_2, uv_3, cond,
                                              conc_b, ph, system_flow, system_pressure, sample_flow,
                                              sample_pressure, prec_pressure, deltac_pressure, postc_pressure,
        ...
        const values = dataArray.map(data => [
            ...
            data.conc_b || null,
            data.ph || null,
            data.system_flow || null,
            data.system_pressure || null,
            data.sample_flow || null,
            data.sample_pressure || null,
            data.prec_pressure || null,
            data.deltac_pressure || null,
            data.postc_pressure || null,
            false,
            new Date()
        ]);

        try {
            // For batch insert, the values array needs to be wrapped in another array
            const [result] = await this.pool.execute(query, [values]);
            logger.info(`✅ Batch inserted ${result.affectedRows} records (${dataArray.length} attempted)`);
            return {success: result.affectedRows, failed: dataArray.length - result.affectedRows};
        } catch (err) {
            logger.error(`❌ Batch insert failed:`, err);

            // If batch fails, try individual inserts
            if (err.code === 'ER_WRONG_VALUE_COUNT_ON_ROW' || err.code === 'ER_PARSE_ERROR') {
                ...
                for (const data of dataArray) {
                    try {
                        await this.insertAktaNodeIds(data);
                        success++;
                    } catch (insertErr) {
                        failed++;
                        logger.error(`Failed to insert result_id ${data.result_id}:`, insertErr.message);
                    }
                }
            ...
            // Fallback to file storage on connection error
            if (this.isConnectionError(err)) {

...
import re
from plotly_integration.models import AktaChromatogram, AktaFraction, AktaRunLog
...
def insert_akta_chromatogram(df, result_id):
    ...
    for _, row in df.iterrows():
        ...
        records.append(AktaChromatogram(
            date_time=timestamp,
            result_id=result_id,
            uv_1_280=row.get("UV_1_280") or row.get("UV_1_280_CUT_TEMP@100,BASEM"),
            uv_2_0=None,
            uv_3_0=None,
            cond=row.get("Cond"),
            conc_b=row.get("Conc_B"),
            pH=row.get("pH"),
            system_flow=row.get("System_flow"),
            system_linear_flow=None,
            system_pressure=row.get("System_pressure"),
            cond_temp=None,
            sample_flow=row.get("Sample_flow"),
            sample_linear_flow=None,
            sample_pressure=row.get("Sample_pressure"),
            preC_pressure=row.get("PreC_pressure"),
            deltaC_pressure=row.get("DeltaC_pressure"),
            postC_pressure=row.get("PostC_pressure"),
            frac_temp=None,
            ml=None
        ))
...
def process_variables_file(variables_file, start_time, end_time, output_dir="test_files"):
    ...
    for result_path, run_vars in result_groups.items():
        ...
        if run_log_var:
            ...
            if batch_id:
                ...
                insert_akta_run_log(run_log_df, batch_id)

...
@app.callback(
    Output("sec-content", "children"),
    Input("url", "search")
)
def display_result(search):
    if not search:
        return html.Div("No result ID provided.")

    query = parse_qs(urlparse(search).query)
    result_id = query.get("result_id", [None])[0]

    if not result_id:
        return html.Div("Invalid result ID.")

    print(f"✅ Test Result ID successful: {result_id}")
    return html.Div(f"Result ID received: {result_id}")

#Populate Table with reports for SEC
@app.callback(
...
# Loading All settings when reopening a report
# @app.callback(
#     Output("channel-checklist", "value"),
#     Output("plot-type-dropdown", "value"),
#     Output("shading-checklist", "value"),
#     Output("peak-label-checklist", "value"),
#     Output("main-peak-rt-input", "value"),
#     Output("low-mw-cutoff-input", "value"),
#     Output("num-cols-input", "value"),
#     Output("vertical-spacing-input", "value"),
#     Output("horizontal-spacing-input", "value"),
#     Output("hmw-column-selector", "value"),
#     Output("standard-id-dropdown", "value"),
...
#     prevent_initial_call=True
# )
# def load_full_plot_settings(report_id):
#     report = Report.objects.filter(report_id=report_id).first()
#     settings = report.plot_settings or {}
#
#     return (
#         settings.get("channel_checklist", ["channel_1"]),
#         settings.get("plot_type", "plotly"),
#         ["enable_shading"] if settings.get("enable_shading", True) else [],
#         ["enable_peak_labeling"] if settings.get("enable_peak_labeling", True) else [],
#         settings.get("main_peak_rt", 7.843),
#         settings.get("low_mw_cutoff", 12),
#         settings.get("num_cols", 3),
#         settings.get("vertical_spacing", 0.05),
#         settings.get("horizontal_spacing", 0.05),
#         settings.get("hmw_columns", [
#             "Sample Name", "HMW", "Main Peak", "LMW", "Calculated MW", "MW Deviation"
#         ]),
#         settings.get("std_result_id", None),
#         settings.get("std_selected_rows", [0, 1, 2, 3]),
#         settings.get("rt_input", None)
#     )

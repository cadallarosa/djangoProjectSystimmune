...
# Define the exact column names as they appear in the Django `LimsFedBatchSamples` model
DB_COLUMNS = {
    "Project ": "project",
...
def confirm_upload(n_clicks):
    ...
    for _, row in df.iterrows():
        ...
        if project_data.get("fb_id") and project_data["fb_id"] is not None:
            LimsUpstreamSamples.objects.update_or_create(
...
def load_existing_data(n_clicks):
    queryset = LimsUpstreamSamples.objects.all().values()

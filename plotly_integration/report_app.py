...
def update_sample_set_options(sample_types):
    ...
    sample_set_names = query.values_list("sample_set_name", flat=True).distinct()
    return [{"label": name, "value": name} for name in sample_set_names if name]
...
def submit_report(n_clicks, report_name, project_id, new_project_id, user_id, new_user_id, comments, table_data, selected_rows):
    if n_clicks > 0:
        ...
        for i in selected_rows:
            ...
            if sample_metadata:
                sample_prefix = sample_metadata.sample_prefix or "N/A"
                sample_number = sample_metadata.sample_number or "N/A"
                selected_samples.append(f"{sample_name}")
        ...
        return f"Report '{report_name}' created successfully with Project ID '{final_project_id}' and User ID '{final_user_id}'!"

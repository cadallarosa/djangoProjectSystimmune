...
all_sensors = ["pir2700", "wir2700", "L/m²/hr", "mL/m²/hr", "mL_min", "flux_decay"]
        ...
        html.Div(
            ...
            children=[
                ...
                dcc.Tabs(id="tabs", value="tab-1", children=[
                    # Tab 1: Select Data
                    dcc.Tab(label="Select Data", value="tab-1", children=[
                        html.Div([
                            html.Label("Select Data to Plot:", style={"fontWeight": "bold"}),
                            dcc.Dropdown(
                                id="data-selection",
                                options=selectable_columns,
                                value=["wir2700", "L/m²/hr"],  # Default values
                                multi=True,
                    ...
                    # Tab 2: Y-Axis Settings
                    dcc.Tab(label="Y-Axis Settings", value="tab-2", children=[
                        html.Div(id="y-axis-settings", style={"padding": "20px"}),
...
# **Generate Y-Axis Settings (All Sensors Visible)**
@app.callback(
    Output("y-axis-settings", "children"),
    Input("data-selection", "value")
)
def generate_y_axis_settings(selected_sensors):
    settings_components = []

    for sensor in all_sensors:  # Show all sensors
        disabled = sensor not in selected_sensors  # Disable if not selected

        settings_components.append(
            html.Div([
                html.Label(f"{sensor} Settings:", style={"fontWeight": "bold"}),
                html.Div([
                    dcc.Input(
                        id={"type": "y-min-input", "index": sensor},
                        type="number",
                        placeholder="Enter Y-min",
                        value=None,
                        style={"marginRight": "10px"},
                        disabled=disabled  # Disable input if sensor is not selected
                    ),
                    dcc.Input(
                        id={"type": "y-max-input", "index": sensor},
                        type="number",
                    ...
                    dcc.Checklist(
                        id={"type": "autoscale-checkbox", "index": sensor},
                        options=[{"label": "Auto-scale", "value": "auto"}],
                        value=[],
...
# **Callback: Disable Y-min & Y-max when Auto-scale is checked**
@app.callback(
    Output({"type": "y-min-input", "index": dash.ALL}, "disabled"),
    Output({"type": "y-max-input", "index": dash.ALL}, "disabled"),
    Input({"type": "autoscale-checkbox", "index": dash.ALL}, "value"),
    prevent_initial_call=True
)
def toggle_y_axis_inputs(autoscale_values):
    return [bool(value) for value in autoscale_values], [bool(value) for value in autoscale_values]
...
@app.callback(
    ...
    Input("data-selection", "value"),
    Input({"type": "y-min-input", "index": dash.ALL}, "value"),  # Dynamically generated inputs
    Input({"type": "y-max-input", "index": dash.ALL}, "value"),  # Dynamically generated inputs
    Input({"type": "autoscale-checkbox", "index": dash.ALL}, "value"),
    Input("smoothing-input", "value"),
    Input("water-flush-flux", "value"),
    prevent_initial_call=True
)
def update_graph(selected_experiment, selected_unit_step, selected_columns, y_mins, y_maxs, autoscale_values,
                 smoothing_seconds, water_flux):
    if not selected_experiment or not selected_unit_step:
        return go.Figure()

    for i, sensor in enumerate(selected_columns):
        y_min = None if "auto" in autoscale_values[i] else y_mins[i]
        y_max = None if "auto" in autoscale_values[i] else y_maxs[i]
    ...
    for i, column in enumerate(selected_columns):
        ...
        if column in data_source.columns:
            ...
            # Handle Auto-scale logic (check if autoscale is enabled for this sensor)
            y_min = None if "auto" in autoscale_values[i] else y_mins[i]
            y_max = None if "auto" in autoscale_values[i] else y_maxs[i]
